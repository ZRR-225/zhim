...
<template>
  <el-dialog
   ...
   <el-form :model="dataForm" ref="dataForm" label-width="100px">
      <el-form-item label="用户ID" prop="maskCode">
        <el-input v-model='dataForm.maskCode' :disabled="true" ></el-input>
      </el-form-item>
    ...
    methods: {
      init (mobile) {
        ...
        this.$nextTick(() => {
			...
			if (data && data.code === 0) {
				this.dataForm.maskCode=data.data.list[0].maskCode
          ...
          if (valid) {
			  this.$http({
				  ...
				  data:this.$http.adornData({
					 ...
					 'userCode':this.dataForm.maskCode,
					 'maskName':this.dataForm.maskName,
				  })
			  }).then(({data})=>{
					if (data && data.code === 0) {
